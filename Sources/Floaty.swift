...
open class Floaty: UIView {
    ...
    fileprivate func setRightBottomFrame(_ keyboardSize: CGFloat = 0) {
        ...
        if superview == nil {
            ...
            frame = CGRect(
                x: ((self.superview!.bounds.size.width - horizontalMargin) - paddingX) + scrollView.contentOffset.x,
                y: ((self.superview!.bounds.size.height - verticalMargin) - paddingY) + scrollView.contentOffset.y,
                width: size,
                height: size
            )
        } else {
